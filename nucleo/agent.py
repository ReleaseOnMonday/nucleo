...
class Agent:
    ...
    def __init__(
        self, config: Optional[Config] = None, bus: Optional["MessageBus"] = None
    ):
        """Initialize agent.
    ...
    async def chat(
        ...
        if metadata:
            ...
            self._current_user_id = (
                ...
                or f"{metadata.get('platform', 'unknown')}:{metadata.get('chat_id', 'unknown')}"
            )
        ...
        if self.memory and self._current_user_id:
            ...
            self.memory.save_memory(
                self._current_user_id, message, tags=tags, importance=1
            )
        ...
        async for chunk in self.llm.chat_stream(
            messages, tool_defs if tool_defs else None
        ):
            if chunk["type"] == "text":
        ...
        if self.memory and self._current_user_id and response_content:
            self.memory.save_memory(
                self._current_user_id, response_content, tags=["response"], importance=1
            )
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"tool_use_id": tool_call["id"], "content": {"error": str(e)}}
    ...
    def _prepare_messages(self) -> List[Dict[str, Any]]:
        ...
        if self.memory and self._current_user_id:
            ...
            recent_memories = self.memory.get_user_memories(
                self._current_user_id, limit=3
            )
        ...
        messages = [{"role": "system", "content": full_system_prompt}] + self.history
    ...
    async def send_to_channel(
        self, content: str, media: Optional[List[str]] = None
    ) -> None:
