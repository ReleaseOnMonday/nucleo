...
╚════════════════════════════════════════════════════════════════╝
    ...
    def configure_llm(self):
        ...
        if choice == "1":
            ...
            api_key = self.get_input(
                "Enter your Claude API key (or press Enter to skip)", default=""
            )
            if api_key:
    ...
    def setup_telegram(self):
        ...
        token = self.get_input(
            "Telegram Bot Token (or press Enter to skip)", default=""
        )
        if token:
            user_ids = self.get_input(
                "Allowed user IDs (comma-separated, or press Enter for any user)",
                default="",
            )
            allowed_users = (
    ...
    def setup_discord(self):
        ...
        if token:
            guild_ids = self.get_input(
                "Allowed Guild IDs (comma-separated, or press Enter for any guild)",
                default="",
            )
            allowed_guilds = (
                [int(x.strip()) for x in guild_ids.split(",") if x.strip()]
                if guild_ids
                else []
            )
    ...
    def configure_tools(self):
        ...
        if self.get_yes_no("  Enable web search tool? (requires BRAVE_SEARCH_API_KEY)"):
            api_key = self.get_input(
                "Brave Search API key (or press Enter to skip)", default=""
            )
            if api_key:
    ...
    def _create_identity_files(self):
        ...
        files = {"IDENTITY.md": "identity", "SOUL.md": "soul", "USER.md": "user"}
