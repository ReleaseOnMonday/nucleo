...
class CronParser:
    ...
    def should_run(cron_parsed: Dict[str, List[int]], dt: datetime) -> bool:
        ...
        """
        return (
            dt.minute in cron_parsed["minute"]
            and dt.hour in cron_parsed["hour"]
            and dt.day in cron_parsed["day"]
            and dt.month in cron_parsed["month"]
            and dt.weekday() in cron_parsed["weekday"]
        )
...
class Task:
    ...
    def __init__(
        self, name: str, schedule: str, action: str, enabled: bool = True, **kwargs
    ):
        """Initialize task.
    ...
    def should_run(self, dt: datetime) -> bool:
        ...
        if self.last_run and self.last_run.replace(
...
class TaskScheduler:
    ...
    def _parse_task_block(self, block: str) -> Optional[Task]:
        ...
        if (
            "task_name" not in fields
            or "schedule" not in fields
        ...
        return Task(
            ...
            **{
                ...
                if k not in ["task_name", "schedule", "action", "enabled"]
            },
        )
